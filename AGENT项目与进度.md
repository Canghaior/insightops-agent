...
| 项目项 | 当前内容 |
|---|---|
| 项目工作名 | InsightOps Agent |
| 中文定位 | 行业情报、经营数据分析与业务执行 Agent |
| 首发场景 | AI / 开源软件行业情报 Agent |
| 长期方向 | 企业经营数据分析与人工确认后的任务执行 |
| 当前阶段 | P2.4-C 已完成并生产验收，进入 10 项目长期稳定性、Alertmanager 与异地恢复演练 |
| 文档版本 | v2.5 |
| 创建日期 | 2026-07-31 |
| 最后更新 | 2026-08-24 |
| 下一评审点 | 10 项目长期稳定性、Alertmanager 外发和异地恢复演练验收后 |

> 当前事实源：`docs/product/complete-agent-product-blueprint.md`、对应架构文档和 `docs/testing/results/` 验收记录。本文第 5～14 节和第 16 节保留早期需求编号与历史计划，未同步勾选的旧复选框不再代表当前完成度；当前状态以第 15 节和完整产品蓝图为准。
...
- [x] `KB-005` 批量生成 Embedding 并写入 pgvector。
- [ ] `KB-006` 支持关键词和向量混合检索。
- [ ] `KB-007` 支持 Rerank、去重和相邻切片合并。
- [ ] `KB-008` 支持文档版本、增量更新和内容哈希。
- [x] `KB-009` 删除文档时删除对应切片和向量。
...
| 阶段 | 状态 | 已完成 | 下一步 |
|---|---|---|---|
| 市场、定位与架构 | 已完成 | InsightOps Agent、首发角色、模块化单体、PostgreSQL + pgvector、独立 Worker | 随生产事实维护蓝图与 ADR |
| P0 工程与最小 Agent | 已完成 | DeepSeek、流式会话、Run/Step/Tool 审计、GitHub Release 工具、CI | 仅维护，不再作为当前阶段 |
| P1 可配置技术情报产品 | 已完成主闭环 | 多来源采集、官方知识库、混合 RAG、反馈复核、报告/Webhook、上传资料、监控备份 | 关闭 10 项目长期稳定性、告警外发和异地恢复门禁 |
| P2.0～P2.3 受控持久 Agent | 已完成并生产验收 | Function Calling、多轮循环、审批写工具、DAG、成本治理、评测发布、跨实例恢复、真实 SIGKILL 演练 | 扩展更多写工具和认证型 MCP 属于后续范围 |
| P2.4-A/B 工作流模板与真实 Run | 已完成并生产验收 | 不可变模板、预检、快照、表达式、持久节点、失败节点重试、运行可视化 | 无阻塞遗留 |
| P2.4-C 工作流产品化 | 已完成并生产验收 | 拖拽 DAG、版本绑定参数预设、模板包、可撤销分享、真实 Run 质量趋势 | 无阻塞遗留 |
| 公开 SaaS | 未开始 | 已有管理员建号、Workspace 隔离和基础配额 | 完成 Go/No-Go 门禁后建设注册、安全、合规与商业闭环 |

### 15.1 当前阻塞项

- P2.4-C 已关闭，无代码级阻塞；两个生产验收导入版本保持 DRAFT 且未激活，仅作为审计证据。
- 10 项目长期稳定性需要生产观察窗口和可审计的成功率样本。
- Alertmanager 外发、异地备份和公开 SaaS 需要明确的外部服务、域名/邮箱及密钥授权；配置必须通过 GitHub Environment Secrets 或服务器受控环境注入。
...
- 修复内部试用发现的 P0 Alpha 安全、页面表达、评测契约和 Release 完整性问题。

#### 已完成

- [x] 前端、后端和 PostgreSQL 默认仅监听 `127.0.0.1`，避免无登录阶段暴露到局域网。
- [x] 新增无外部依赖的安全 Markdown 渲染，支持粗体、列表、代码和 HTTP/HTTPS 链接，并转义原始 HTML。
- [x] 修正概览页“持续追踪”和“DeepSeek 调用工具”等超出实际链路的描述。
- [x] 20 问评测集逐条校验运行时项目识别、工具路由和时间窗口；修正未明确列出比较项目的问题。
- [x] 自然语言“最近/近期/目前”统一采用 90 天 Release 口径，时间窗口问题每项目最多读取 30 条。
- [x] GitHub Release Gateway 支持翻页；达到证据上限时明确标记截断，禁止模型声称完整统计。

#### 验证

- Maven Reactor 完整 `verify` 共 45 个测试全部通过，数据库门禁 0 跳过。
- 前端 lint、9 个测试和生产构建全部通过。
- 页面复查确认历史回答 Markdown 正常渲染、概览文案准确、浏览器控制台无错误。
- 15173、18080、55432 均只监听 `127.0.0.1`；数据库历史卷和原有 Run 记录保留。

#### 下一步

- [ ] 继续三角色内部模拟与真实用户 Alpha 验收，记录任务完成率和共同痛点。
- [ ] P1 实现基础登录、工作区隔离和多轮上下文记忆。

### 2026-08-16 / v0.11

#### 本次目标

- 修复研究问答第二轮无法理解“这个版本”等指代的问题，并完成真实在线回归。

#### 已完成

- [x] 后端按 `sessionId` 读取最近 12 条用户/助手消息并作为受限历史上下文传给模型。
- [x] Release 工具在当前问题未写项目名时，仅继承最近一条用户问题中的白名单项目；显式切换项目时以当前问题为准。
- [x] 前端在当前标签页保存会话 ID，连续发送沿用同一会话，并提供“新建会话”入口。
- [x] 页面明确区分 P0 短期上下文与 P1 账号级、跨设备、可管理的长期记忆。

#### 验证

- Maven Reactor 完整 `verify` 共 47 个测试全部通过，数据库门禁 0 跳过；前端 lint、9 个测试和生产构建全部通过。
- 真实浏览器两轮验证通过：先问 Spring AI 最新稳定版本，再问“这个版本相比上一个版本有什么变化？”，第二轮正确比较 v2.0.0 与 v1.1.8。
- 两次 Run 均为 `SUCCEEDED`、`toolRounds=1`，且共享同一个 `sessionId`；15173、18080、55432 均只监听 `127.0.0.1`。

#### 下一步

- [ ] 由项目负责人继续完成三角色 Alpha 用户验收并记录问题。
- [ ] P1 实现基础登录、工作区隔离，以及账号级、跨设备和可管理的长期记忆。

### 2026-08-16 / v0.12

#### 本次目标

- 修复聊天输入框不清空和页面只能显示当前一轮问答的问题，并重新核对第一阶段完成度。

#### 已完成

- [x] 发送问题后立即清空输入框；连接建立前失败时恢复原输入，避免用户内容丢失。
- [x] 输入框支持 `Enter` 发送、`Shift + Enter` 换行，并兼容 `Ctrl + Enter` 发送。
- [x] 聊天页改为可滚动的多轮消息列表，USER/ASSISTANT 按数据库顺序连续展示。
- [x] 新增只读会话历史接口，当前标签页刷新后从 PostgreSQL 恢复最近 100 条消息。
- [x] 流式回答追加到当前助手消息；用户阅读较早内容时不强制滚动，接近底部时自动跟随。
- [x] 修正中文右括号被包含进裸链接 URL 的渲染问题。

#### 验证

- Maven Reactor 完整 `verify` 共 49 个测试全部通过，数据库门禁 0 跳过。
- 前端 lint、4 个文件共 12 个测试和生产构建全部通过。
- 真实浏览器验证通过：发送后输入框为 `0/4000`；两轮问题和回答同屏；刷新后恢复 4 条数据库消息。
- 历史接口返回同一会话的 `USER,ASSISTANT,USER,ASSISTANT`，最新 Run 为 `SUCCEEDED` 且 `toolRounds=1`。
...
- 完成 P0 Guardrail、三角色 Alpha 验收和推送前收尾。

#### 已完成

- [x] 实现输入规范化、不可信上下文边界、敏感信息保护、仓库白名单和官方 Release 来源校验。
- [x] 去除不可信 Release 正文中的外部链接目标，回答正文只链接可信 GitHub Release tag URL。
- [x] 多轮 Release 追问同时继承项目和显式时间窗口，并由服务端提供每项目证据计数。
- [x] 完成 Java 开发者、架构师、技术负责人共 9 个真实页面任务；提示注入和非白名单仓库访问被拒绝。
- [x] 将共同问题、修复过程和 P1 产品判断写入用户模拟与测试结果文档。

#### 验证

- Maven Reactor 完整 `verify` 共 57 个测试全部通过，数据库门禁 0 跳过。
- 前端 lint、4 个文件共 12 个测试和生产构建全部通过。
...
- 将长期进度文档与完整产品蓝图、P2.4-B 生产事实和最新持久聊天续流修复统一。

#### 已完成

- [x] P2.0～P2.3 的受控多轮、可靠性、评测治理和跨实例恢复均完成生产验收。
- [x] P2.4-A/B 的模板、不可变快照、真实持久 Run、节点恢复和失败节点定点重跑完成生产验收。
- [x] 修复持久聊天在浏览器中等待但刷新后才显示答案的问题，用户确认复杂问题现在可自动显示答案。
- [x] 明确下一阶段依次为 P2.4-C、10 项目长期稳定性/Alertmanager/异地恢复、公开 SaaS 门禁与实施。

#### 验证

- 后端真实 PostgreSQL 18.4：273/273，Flyway 36/36。
- 前端：22 个测试文件、57/57，lint、类型检查和生产构建通过。
- 当前生产提交：`60c94ab0e0cddca08beef1c61f38446de830bb18`。
- CI Run `32708453090` 与部署 Run `32708756290` 成功。
- 生产服务 `UP`，DeepSeek `deepseek-v4-flash ready=true`，聊天页 HTTP 200，未登录事件接口返回 401。
...
- 完成 P2.4-C 工作流产品化、全套门禁、生产发布和认证态验收。

#### 已完成

- [x] 可拖拽 DAG、可视依赖和不可变版本布局元数据。
- [x] 用户级、活动版本绑定并经服务端类型校验的参数预设。
- [x] 模板包导入导出、有期可撤销分享、令牌哈希和导入审计。
- [x] 真实 Run、节点、反馈、引用、Token、费用和耗时质量趋势。
- [x] 分享令牌从请求 URL 移到 fragment/POST body；多成员反馈按 Run 去重聚合。

#### 验证

- 后端全仓 280/280，Failures 0、Errors 0、Skipped 0；PostgreSQL 18.4、Flyway 37/37。
- 前端 23 个测试文件、63/63，lint、类型检查和生产构建通过。
- 生产代码 `9563234ec10b3a3efee78f6f6c5f05c3367b6151`；CI `32716387888`、Deploy `32716895893` 成功。
- 公开页面 200、服务 UP、DeepSeek ready、未登录 P2.4-C API 401。
- 替代认证态验收 Run `32717865872` 成功；验收脚本修复提交 `f64e083c531dee4ad76afec6392e2f735f78bbbd` 的 CI `32717652958` 全绿。

#### 下一步

- [ ] 完成 10 项目长期稳定性、Alertmanager 外发和异地恢复演练。
- [ ] 在上述生产可靠性门禁关闭后完成公开 SaaS Go/No-Go，并在 Go 时实施公开运营能力。
