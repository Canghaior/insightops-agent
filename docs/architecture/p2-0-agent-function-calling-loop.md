...
Executor 每轮最多执行一个工具。模型返回多个候选调用时，循环按模型顺序选择第一个尚未执行的调用，其余候选延后到后续规划轮次；相同工具与完全相同参数的重复调用不会再次执行，而是以失败 Observation 返回给下一轮。参数 JSON 限制为 64,000 字符，单轮返回给模型的 Observation 限制为 12,000 字符，最终答案累计证据限制为 48,000 字符。
...
- 每轮最多执行一个工具；模型同时建议多个工具时安全顺序化，不并行执行。
