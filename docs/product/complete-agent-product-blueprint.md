...
| 可公开运营的 SaaS | 约 82% | P3.1 身份与团队、P3.2 免费公开注册/数据权利、P3.4 单机可运营边界和供应链代码已完成；仍待外部邮件/运营者配置、真实生产验收、专业法律审阅及未来收费能力 |

当前最准确的阶段判断是：

> 已经上线并可真实使用的受控技术情报 Agent；免费公开 Beta 的产品代码已就绪，但生产注册仍安全关闭，等待邮件模板/CAM/运营者配置和真实注册全链路验收。

2026-08-26 更新结论：当前封闭 Alpha/管理员邀请制受控 Beta 为 `GO`；免费公开 Beta 已达到代码就绪但仍为 `CONDITIONAL GO`，须完成腾讯云邮件模板/CAM、运营者信息、Turnstile Secret、真实邮箱注册全链路和生产容量 Smoke 后才能开闸。付费 SaaS 主动延期，仍为 `NO-GO`；详见 `docs/product/public-saas-go-no-go-2026-08-24.md` 与 P3.2/P3.4 架构文档。
...
- 已从删除首份 Runner 副本后的 Artifact 往返副本完成 PostgreSQL 18 隔离恢复，验证当时 Flyway 37、10 项目、Workspace、用户和上传摘要；P3.2/P3.4 当前数据库版本为 Flyway 40。
...
### 5.7 身份安全与发布供应链已形成免费 Beta 闭环

P3.1 已增加 TOTP/一次性恢复码、邮箱验证和枚举安全的密码找回、设备 Session 与全会话撤销、PostgreSQL 持久限流、一次性哈希令牌、AES-256-GCM Secret/邮件 Outbox、写 API CSRF/Origin 门禁和 HSTS。P3.2/P3.4 又增加 Turnstile 服务端校验、公开注册双开关、数据库 Run 停止兜底、个人数据加密导出和个人 Workspace 级联删除。

发布供应链已完成：

- CodeQL Java 与 JavaScript/TypeScript SAST。
- Dependency Review 和 Gitleaks。
- Trivy 源码与生产镜像漏洞扫描。
- Maven/npm CycloneDX SBOM。
- GitHub OIDC keyless cosign 镜像 digest 签名。

仍未完整覆盖：

- Secret 定期轮换和轮换演练。
- 公开规模扩大后的 WAF、CDN、DDoS 增强。
- 正式安全告警分级、响应和复盘演练。
...
### 6.6 模型治理已完成主闭环，路由与多供应商降级仍待补齐

当前已能查看生产模型与就绪状态，模型超时、错误分类、重试和熔断已进入 Agent 运行时；Prompt、模型、参数和工具合同支持不可变候选版本、评测门禁与原子激活，Run 与 Workspace 页面记录 Token、费用和调用情况。完整产品仍需要：

- 主模型和备用模型的多供应商配置。
- 按聊天、规划、总结、评测等任务选择模型。
- 主模型不可用时基于健康状态自动切换并安全回切。
- 管理员模型路由页面和不暴露密钥的可用性测试。
- 用户级套餐配额和商业用量边界。

### 6.7 免费公开注册与数据权利代码已完成，生产仍待外部配置

P3.1 已完成邀请、邮箱验证、忘记密码/重置、TOTP MFA、设备 Session 和账户删除宽限期。P3.2 已增加最多 100 人的 14+ 自主注册、原子名额、未验证过期释放、Turnstile、腾讯云 SES、版本化同意、个人数据加密导出、个人 Workspace 级联删除和备份过期说明。

当前生产仍保持注册关闭，待三个腾讯云模板审核、最小权限 CAM 凭据、运营者名称/公开联系邮箱和 Turnstile Secret 配置后完成真实邮箱端到端验收。

项目法律页面反映当前真实数据流和免费 Beta 边界，但没有宣称专业法律审阅。正式扩大公开运营前仍建议进行适用法规复核，并建立条款变更和重新同意流程。
...
- [ ] 关键结论官方来源覆盖率达到 100%。
- [ ] 采集成功率达到 95% 以上。
- [x] 用户可以完成登录、连续提问、查看历史、查看引用和查看执行记录（生产已验收，持久续流修复后用户再次确认无需刷新即可获得答案）。
- [ ] 每日备份、异地副本和恢复演练形成记录。
...
- [~] 邀请制与公开注册、验证、找回、MFA、Session、数据导出和公开个人账号级联删除已完成（P3.1/P3.2）；生产开闸待外部配置和真实邮箱验收。
- [~] 免费 Beta 并发/容量/用量边界完整；套餐、订单、支付、退款、发票作为未来收费阶段延期。
- [~] 用户协议、隐私政策、可接受使用政策和版本化同意已实现；仍待运营者信息和专业法律审阅。
- [x] 异地备份、恢复演练和外部告警完整（Stage 3 生产验收已通过）。
- [~] CodeQL、依赖/Secret/镜像扫描、SBOM 和镜像签名已完成；WAF/CDN 与 Secret 轮换仍待规模扩大前增强。
- [~] 单机 best-effort 目标、双停止开关、告警、容量脚本和运行手册已完成；真实生产容量与注册全链路验收仍待执行。
...
### 阶段 D：补齐公开运营能力（免费 Beta 条件开放，收费 NO-GO）

2026-08-26：P3.1 已完成生产验收；P3.2/P3.4 的免费公开 Beta、数据权利、单机运行和供应链代码已完成本地真实 PostgreSQL/应用上下文验收。生产继续保持注册关闭，待运营者补齐邮件模板/CAM/联系信息后执行真实邮箱和容量验收；本阶段不收费。

1. [x] 完成邀请制邮箱身份、验证、找回密码、TOTP MFA、Session 和账户注销申请（P3.1；不开放自主注册）。
2. [x] 完成 Workspace 创建/切换、资料、邀请、Owner/Member、所有权移交、成员退出和归档（P3.1）。
3. [~] Agent 基础配额与用量已完成；免费 Beta 暂不设月额度，套餐、订单、支付、退款、发票和财务计费主动延期。
...
6. [~] CodeQL、依赖/Secret/镜像扫描、SBOM、镜像签名和真实数据流合规页面已完成；WAF/CDN、Secret 轮换和专业法律审阅待后续增强。
7. [~] 完成最多 100 人、14+、Turnstile、腾讯云 SES、原子名额、版本化同意、数据导出和个人账号级联删除（P3.2；代码完成，生产开闸待外部配置）。
8. [~] 完成单机双停止开关、6 条公开 Beta 告警、容量基线、发布预检和运行手册（P3.4；代码完成，生产容量验收待执行）。
