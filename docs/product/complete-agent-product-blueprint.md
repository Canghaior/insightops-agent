...
| 通用多工具自主 Agent | 约 80% | P2.0 已完成 Function Calling、可靠性和写工具治理；P2.1-A/B/C 已完成生产验收；P2.2-A/B/C 已提交并部署，CI、生产部署与公网只读验收通过，Owner 登录态业务验收待关闭 |
...
- GitHub `production` Environment Secrets 已完成配置；P2.2 CI Run `32552306868` 和生产部署 Run `32552470905` 均成功，生产功能版本为 `0b0d2d9e123d2dc3e6bcba4a6daa5fd2a7c6a904`。
...
- [x] 不可变版本化评测集覆盖工具选择、失败恢复、引用、耗时、Token 与成本门槛（P2.2-A，CI 与生产部署通过）。
- [x] Prompt、模型、参数和工具合同作为候选版本管理，只有通过指定评测基线才能原子激活（P2.2-B，CI 与生产部署通过）。
- [x] Owner/系统管理员可运行评测、比较趋势、查看逐案例 Trace，并从失败 Run 沉淀回归案例（P2.2-C，页面已部署，登录态业务验收待关闭）。
...
9. [x] 实现版本化评测集、真实 AgentLoop 离线评测、质量/成本门槛、候选版本和生产激活审计（P2.2-A/B/C，CI、部署与公网只读验收通过，登录态业务验收待关闭）。
